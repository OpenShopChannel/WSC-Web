...
<a class="btn<#if class?has_content> ${class}</#if>"<#if id?has_content> id="${id}"</#if><#if href?has_content> href="${href?html}"</#if><#if w?has_content || h?has_content || style?has_content> style="<#if w?has_content> width: ${w};</#if><#if h?has_content> height: ${h};</#if><#if style?has_content> ${style}</#if>"</#if>>
